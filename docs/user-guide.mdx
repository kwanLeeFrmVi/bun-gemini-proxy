...
- ✅ **Zero-downtime key rotation** with health scoring
- ✅ **Streaming and non-streaming** responses
- ✅ **Automatic retry** with exponential backoff
- ✅ **Sub-100ms proxy overhead** under normal load
- ✅ **Hot configuration reload** without restart
- ✅ **Comprehensive observability** with metrics and structured logs
...
**Features**:
- ✅ Intelligent key rotation across multiple Gemini API keys
...
**Features**:
- ✅ Uses local `gemini` CLI tool
- ✅ Automatically loads GEMINI.md project context
- ✅ No API key rotation (single CLI session)
- ❌ **No streaming support** (CLI limitation)
...
**Features**:
- ✅ Uses local `codex` CLI tool (Claude Code CLI)
- ✅ Agent-aware responses with reasoning
- ✅ Extended timeout (120s) for complex operations
...
**Admin Endpoints**: Require admin token (configured in `config/proxy.yaml`)
...
- ⏱️ 60-second timeout (includes GEMINI.md loading)
...
**Limitations**:
- ❌ No streaming support (`stream: true` returns error)
- ⏱️ 120-second timeout (agent operations take longer)
...
Admin endpoints require authentication via `adminToken` configured in `config/proxy.yaml`.
...
- `accessTokens: []` + `requireAuth: false` → No authentication
- `accessTokens: []` + `requireAuth: true` → No authentication (auto-disabled)
- `accessTokens: [...]` + `requireAuth: true` → Authentication required
- `accessTokens: [...]` + `requireAuth: false` → Authentication optional

### API Keys Configuration (`config/keys.yaml`)
...
### Monitoring Configuration (`config/proxy.yaml`)
...
### Persistence Configuration (`config/proxy.yaml`)
...
Adjust circuit breaker sensitivity in `config/proxy.yaml`:
...
**Reloadable**:
- ✅ API keys (add/remove/update)
- ✅ Key weights and cooldowns
...
vim config/proxy.yaml  # Change port: 8000 to port: 8001
```
...
4. **Configuration**: Review `config/*.yaml` for misconfigurations
